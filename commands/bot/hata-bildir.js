...
module.exports = class TavsiyeCommand extends Command {
    constructor(client) {
        super(client, {
            name: 'hata-bildir',
            group: 'bot',
            memberName: 'hata-bildir (hata)',
            description: 'Bottaki Bir Hatayı Bildirirsiniz.',
            args: [
                {
                    key: 'hata',
                    prompt: 'Bildirmek İstediğiniz Hatayı Yazınız.',
                    type: 'string'
                }
            ]
        });
...
async run(message, args) {
    ...
    message.reply(`${this.client.emojis.get('464406478153973770')} Hata Bildirildi! Yakında İncelenecektir!`);
